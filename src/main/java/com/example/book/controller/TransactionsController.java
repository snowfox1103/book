...
public class TransactionsController {
    ...
    public void getTransList(PageRequestDTO pageRequestDTO, Model model){
        log.info("--------get list---------");
    ...
    public void getTransRegister(Model model){
        List<Categories> categories = categoriesService.categoriesList();
        model.addAttribute("categories", categories);
    ...
    public void read(Long tno,PageRequestDTO pageRequestDTO,Model model){ //url에서 tno값 받아오기, model 이용해서 view(html,jsp 등)에 전달
        List<Categories> categories = categoriesService.categoriesList();
        model.addAttribute("categories", categories);
    ...
    public String transModify(@Valid TransactionsDTO transactionsDTO,PageRequestDTO pageRequestDTO,BindingResult bindingResult,RedirectAttributes redirectAttributes){
        ...
        if (bindingResult.hasErrors()){
            ...
            return "redirect:/trans/transModify44444?"+link;
        }
